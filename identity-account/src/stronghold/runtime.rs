...
impl ActorState {
  ...
  pub(crate) async fn flush(&mut self, path: &Path, persist: bool) -> Result<()> {
    ...
    for client in self.clients_loaded.iter() {
      self.stronghold.kill_stronghold(client.clone(), false).await.to_result()?;
      self.stronghold.kill_stronghold(client.clone(), true).await.to_result()?;
    }
  ...
  pub(crate) async fn load_actor(&mut self, snapshot: &Path, client: &[u8], flags: &[StrongholdFlags]) -> Result<()> {
    ...
    if self.clients_active.contains(client) {
      ...
      self.stronghold.spawn_stronghold_actor(client.into(), flags.to_vec()).to_result()?;
...
impl Task {
    ...
    T: Send + 'static,
  {
    ...
    let join: JoinHandle<Result<T>> = guard.spawn(future);
...
impl Passwords {
  ...
  fn get() -> &'static Self {
    ...
    let this: &Self = __THIS.get_or_init(|| {
      Self {
